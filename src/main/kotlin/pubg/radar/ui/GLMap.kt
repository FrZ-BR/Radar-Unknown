...
class GLMap: InputAdapter(), ApplicationListener, GameListener {
  ...
  override fun create() {
    ...
    val generatorHub = FreeTypeFontGenerator(Gdx.files.internal("fonts/AgencyFB.ttf"))
    ...
    val generator = FreeTypeFontGenerator(Gdx.files.internal("fonts/ArialNarrowBold.ttf"))
  ...
  */
  ...
  private fun ShapeRenderer.drawItem() {
    droppedItemLocation.values.asSequence().filter { it.second.isNotEmpty() }
      .forEach {
        ...
        if ("helmet3" in items || "armor3" in items || "8x" in items || "bag3" in items || "4x" in items || "heal" in items || "drink" in items) {
          color = BLACK
          rect(x - 4 - backgroundRadius, y - 4 - backgroundRadius, backgroundRadius * 2, backgroundRadius * 2)
          color = finalColor
          rect(x - 4 - radius, y - 4 - radius, radius * 2, radius * 2)
  ...
  fun drawPlayerInfos(players: MutableList<renderInfo>?) {
    players?.forEach {
      ...
      if (completedPlayerInfo.containsKey(name)) {
        ...
        val desc = "$teamNumber($enemyDistance)\n${info.killDeathRatio.d(2)} / ${info.headshotKillRatio.d(2)}"
        for(i in -1..1)
          for(j in -1..1)
        ...
        for(i in -1..1)
          for(j in -1..1)
            nameFontShadow.draw(spriteBatch, "$teamNumber  $enemyDistance", sx + 2 + i, windowHeight - sy - 6 + j)
        nameFont.draw(spriteBatch, "$teamNumber  $enemyDistance", sx + 2, windowHeight - sy - 6)
      }
      */
  ...
  fun ShapeRenderer.drawPlayer(pColor: Color?, actorInfo: renderInfo, drawSight: Boolean = true) {
    ...
    if ((airx < 0 || airx > windowWidth || airy < 0 || airy >windowHeight) && enemyDistance < 500) {
      var inWindowX = 0f
      var inWindowY = 0f
      if (airx < 0 && (windowHeight/2 - airy)/(windowWidth/2 - airx) < windowHeight/windowWidth && windowHeight/2 > airy) {
        inWindowX = 0f
